...
from StudyX import YouTube, app
from StudyX.core.call import Study
from StudyX.misc import SUDOERS, db
from StudyX.utils import bot_sys_stats
from StudyX.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from StudyX.utils.formatters import seconds_to_min
from StudyX.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from StudyX.utils.stream.autoclear import auto_clean
from StudyX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Study.pause_stream(chat_id)
        ...
        await Study.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Study.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Study.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Study.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Study.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
